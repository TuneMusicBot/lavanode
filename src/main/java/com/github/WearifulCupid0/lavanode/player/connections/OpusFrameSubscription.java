...
public final class OpusFrameSubscription implements AutoCloseable {
    private final OpusFrameDispatcher dispatcher;
    private final int capacity;
    private final Deque<byte[]> frames;
    private final AtomicBoolean closed = new AtomicBoolean();
    private volatile long lastDemandNanos;

    OpusFrameSubscription(OpusFrameDispatcher dispatcher, int capacity) {
        if (capacity < 1) {
    ...
    boolean hasRecentDemand(long nowNanos) {
        ...
        return demand != 0L && nowNanos - demand <= OpusFrameDispatcher.DEMAND_TIMEOUT_NANOS;
    }
    ...
    private void signalDemand() {
        ...
        synchronized (frames) {
            if (lastDemandNanos == 0L || now - lastDemandNanos > OpusFrameDispatcher.DEMAND_TIMEOUT_NANOS) {
                frames.clear();
            }
            lastDemandNanos = now;
        }
